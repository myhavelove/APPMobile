...
<script>
	export default {
		...
		props: {
			...
			url: {
				type: String,
				default: getApp().globalData.apiHost+'CoopDr/ImgUpload'
		...
		methods: {
			...
			chooseImage() {
				...
				uni.chooseImage({
					...
					sizeType: ['compressed', 'original'],//是否压缩
					sourceType: ['album', 'camera'],//选择图片路径（拍照/相册）
					success: function(res) {
						...
						_self.imageindex=_self.upimg_seat;
		...
		const uploadTask = uni.uploadFile({
			...
			success: function(res) {
				...
				try {
					...
					data = JSON.parse(res.data).data.path+','+extra;
				} catch (e) {
